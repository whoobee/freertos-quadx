...
void Main_5ms_task(void *pvParameters)
{
    for (;;)
    {
        if (mIC4CaptureReady())
        {
            CaptureTimeOld = CaptureTime;
            CaptureTime = (unsigned short) mIC4ReadCapture();
            UpdatedTime = TRUE;
        }

        if (UpdatedTime == TRUE)
        {
            if (CaptureTime > CaptureTimeOld)
            {
                DeltaTime = CaptureTime - CaptureTimeOld;
                UpdatedTime = FALSE;
            }
            else
            {
                DeltaTime = (CaptureTime + (0xFFFF - CaptureTimeOld));
                UpdatedTime = FALSE;
            }
        }
        Update_Status_LED(DeltaTime);
